...
import { editJob } from '../../redux/actions/jobsActions';
...
function EditJob({ match }) {
    ...
    const postAllJobAndCompanyInfoHandler = values => {
        ...
        finalPostObj._id = match.params.id;
        dispatch(editJob(finalPostObj));
    }
    const { jobs } = useSelector(state => state.jobsReducer);
    const jobToEdit = jobs.find(job => job._id === match.params.id)
    return (
        <MainLayout>
            ...
            <Tabs defaultActiveKey="1" className='postnewjob' activeKey={activeTabKey}>
                <TabPane tab="Job Info" key="1">
                    <Form className='m-3' layout="vertical" onFinish={jobInfoSaveHandler} initialValues={jobToEdit}>
                        <Row gutter={20}>
                ...
                <TabPane tab="Company Info" key="2">
                    <Form
                        className='m-3'
                        layout="vertical"
                        onFinish={postAllJobAndCompanyInfoHandler}
                        ...
                        <Button type="primary" htmlType='submit'>Save &#38; Update Job</Button>
                    </Form>
                </TabPane>
            </Tabs>
        </MainLayout>
